...
import { handleOnBlur, handleOnFocus, parsePassword } from '../common'
import { showError, Progressing, ErrorScreenManager, RadioGroup, RadioGroupItem, CustomInput } from '@devtron-labs/devtron-fe-common-lib'
            ...
            promise
                .then((response) => {
                    ...
                    if (errorMap != null && Object.keys(errorMap).length == 0) {
                        ...
                        {this.state.selectedRepoType === repoType.DEFAULT && toast.error('Configuration failed')}
                        {this.state.selectedRepoType === repoType.CONFIGURE && toast.success('Configuration saved successfully')}
                    }
                })
                .catch((error) => {
                    showError(error)
