...
export function getData() {
    return Promise.all([getTeamListMin(), getAppListMin(), getEnvironmentListMin()])
        .then(([teamList, appList, environmentList]) => {
...
export function getParsedData(teamList, appList, environmentList) {
    let lists = {
        team: teamList.result ? teamList.result.map((team) => { return { value: team.name, name: team.name } }) : [],
        application: appList.result ? appList.result.map((app) => { return { value: app.name, name: app.name } }) : [],
        environment: environmentList.result ? environmentList.result.map((env) => { return { value: env.environment_name, name: env.environment_name } }) : [],
...
export function userModal(user) {
    return {
        ...
        rules: user.roleFilters ? user.roleFilters.map((rule) => {
            return {
                team: rule.team || "",
