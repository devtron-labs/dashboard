...
const MultiChartSummary: React.FC<MultiChartSummaryProps> = ({ charts, getChartVersionsAndValues, toggleChart, configureChart, handleChartValueChange, handleChartVersionChange, chartListing, configureChartIndex, removeChart, hideDeployedValues }) => {
    ...
    return (
        <div className="chart-group--summary">
            {chartListing && <div className={`selected-chart-widget p-12 select-chart cursor ${typeof configureChartIndex !== 'number' ? 'active' : ''}`} style={{ gridTemplateColumns: "40px 1fr 24px" }} onClick={chartListing}>
...
const SelectedChartWidget: React.FC<SelectedChartWidget> = ({ chart, remove, selectChart, selectChartVersion, selectChartValues, toggleChart, getChartVersionsAndValues, selected, hideDeployedValues }) => {
    ...
    return (
        <div className={`selected-chart-widget pointer ${selected ? 'active' : ''} ${chart?.name?.error || chart?.environment?.error ? 'selected-chart-widget--error' : ''}`}>
            ...
            {remove
                ? <div className="trash-container"><Trash className="icon-dim-20 cursor" onClick={remove} /></div>
                : toggleChart
