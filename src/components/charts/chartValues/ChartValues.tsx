...
import { getChartValues, updateChartValues, createChartValues, getChartVersionsMin, getChartVersionDetails, deleteChartValues } from '../charts.service';
...
export default class ChartValues extends Component<ChartValuesProps, ChartValuesState> {
    ...
    componentDidMount() {
        getChartVersionsMin(this.props.match.params.chartId).then((response) => {
            ...
            if (this.props.match.params.chartValueId) {
                getChartValues(this.props.match.params.chartValueId, 'TEMPLATE').then((response) => {
                    this.setState({
                ...
                getChartVersionDetails(chartVersionId).then((response) => {
                    this.setState({
    ...
    handleChartVersionChange(event: React.ChangeEvent<HTMLFormElement>): void {
        ...
        getChartVersionDetails(event.target.value).then((response) => {
            this.setState({
                values: response.result.rawValues,
                view: ViewType.FORM
            })
    ...
    deleteChartValue(event: React.MouseEvent<HTMLButtonElement>) {
        deleteChartValues(parseInt(this.props.match.params.chartValueId)).then((response) => {
    ...
    render() {
        let selectedChartVersion = this.state.chartVersions.find(chartVersion => chartVersion.id === this.state.chartVersionId);
                ...
                <Header appStoreApplicationName={this.state.appStoreApplicationName}/>
                    <form className="chart-values__container">
                        ...
                        <div className="form__row">
                            ...
                            <Select tabIndex={2} value={this.state.chartVersionId} onChange={this.handleChartVersionChange}>
                                ...
                                {this.state.chartVersions.map((chartVersion, index) => {
                                    return <Select.Option key={`${chartVersion.version}`} value={chartVersion.id}>{chartVersion.version}</Select.Option>
                                })}
                            </Select>
                        </div>
                        <div className="mb-12 code-editor-container">
                            <CodeEditor value={this.state.values} noParsing
                                mode="yaml"
                        ...
                        <div className="flex right">
                            {this.props.match.params.chartValueId
                                ? <button type="button" className="cta delete mr-16" onClick={this.deleteChartValue}>Delete</button>
                                : <button type="button" className="cta cancel mr-16" onClick={this.redirectToDiscoverChart}>Cancel</button>}
                            <ButtonWithLoader isLoading={this.state.buttonLoader}
                                rootClassName="cta"
                                loaderColor="white"
...
function Header({appStoreApplicationName}){
    const {breadcrumbs} = useBreadcrumb({alias:
        {
    ...
    return(
        <div className="page-header">
            <div className="flex left column">
                ...
                <div className="flex left page-header__title">Create Custom Values</div>
            </div>
