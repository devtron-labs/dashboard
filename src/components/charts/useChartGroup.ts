...
export default function useChartGroup(chartGroupId = null): ChartGroupExports {
    ...
    async function getChartVersionsAndValues(chartId: number, index: number): Promise<void> {
        try {
            ...
            const { result: { values: chartValuesList } } = await getChartValuesCategorizedList(chartId);
    ...
    async function validateData() {
        try {
            ...
            let tempCharts = state.charts.map(chart => {
                ...
                return {
                    ...chart,
                    name: { value: chart.name.value, error: nameRegexp.test(chart.name.value) ? '' : 'name must follow `^[a-z]+[a-z0-9\-\?]*[a-z0-9]+$` pattern' },
                    environment: { ...chart.environment, error: chart?.environment?.id ? '' : 'Environment is mandatory' }
                }
            })
            if (!validated) {
                setState(state => ({ ...state, charts: tempCharts }))
    ...
    async function fetchChartValues(chartId, index): Promise<void> {
        try {
            const { result: { values: availableChartValues } } = await getChartValuesCategorizedList(chartId)
            const tempCharts = [...state.charts]
    ...
    async function configureChart(index: number) {
        ...
        try {
            ...
            if (tempCharts[index].appStoreValuesVersionId) {
                const { result: chartVersionDetails } = await getChartValues(tempCharts[index].appStoreValuesVersionId, tempCharts[index].kind || 'TEMPLATE')
                tempCharts[index] = { ...tempCharts[index], originalValuesYaml: chartVersionDetails.values, valuesYaml: chartVersionDetails.values, loading: false }
            }
            else {
                const { result: chartVersionDetails } = await getChartValues(tempCharts[index].appStoreApplicationVersionId, tempCharts[index].kind)
                tempCharts[index] = { ...tempCharts[index], originalValuesYaml: chartVersionDetails.values, valuesYaml: chartVersionDetails.values, loading: false }
            }
        }
        catch (err) {
    ...
    async function handleChartValueChange(index: number, kind: 'DEPLOYED' | 'DEFAULT' | 'TEMPLATE' | 'EXISTING', valuesId: number) {
        ...
        try {
            ...
            tempCharts[index] = { ...tempCharts[index], kind, appStoreValuesVersionId: valuesId, originalValuesYaml: chartVersionDetails.values, valuesYaml: chartVersionDetails.values, loading: false }
            setState(state => ({ ...state, charts: tempCharts }))
        }
        catch (err) {
            showError(err)
    ...
    async function createChartValues(index: number, name: string) {
        ...
        try {
            const { result: { id, appStoreVersionId, name: newName, values: newValues } } = await createChartValuesService({
