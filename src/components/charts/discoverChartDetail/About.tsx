...
export function About({ description = "", home = "", chartYaml, appVersion = "", created = "", digest = "", source = "", isOCICompliantChart=false, ...rest }) {
    return <div className="flex column left about white-card white-card--no-padding">
        ...
        <div className="chart-store-card__body">
            ...
            <div className="mb-16">
                <a rel="noreferrer noopener" className="chart-store-card__text homepage anchor" href={home} target="_blank" data-testid="home-chart-deploy-link">{home ? home : "-"}</a>
            </div>
...
function Detailed({ appVersion = "", created = "", digest = "", home = "", source = "", chartYaml, isOCICompliantChart, ...rest }) {
    ...
    return (
        <div className="detailed-container flex column left detailed">
            ...
            <div className="chart-store-card__text chart-store-card__text--see-more pointer anchor flex" onClick={e => toggleDetailed(d => !d)} data-testid="extend-read-more"><span className="cb-5">{detailed ? 'Read less' : 'Read more'}</span><DropdownIcon className="rotate" style={{ ['--rotateBy' as any]: `${Number(detailed) * 180}deg` }} /></div>
        </div>
    )
}
