...
export default function DeployedChartFilters({
    handleFilterQueryChanges,
    appStoreName,
    searchApplied,
    handleCloseFilter,
    handleAppStoreName,
    ...
    function keys(key) {
        if (key == 'repository') {
            handleFilterQueryChanges(selectedChartRepo, 'repository')
        }
        if (key == 'environment') {
            handleFilterQueryChanges(selectedEnvironment, 'environment')
        }
    }
        ...
        return (
            <components.MenuList {...props}>
                ...
                <div className="chart-list-apply-filter flex bcn-0 pt-10 pb-10">
                    <button
                        type="button"
                        className="cta flex cta--chart-store"
                        disabled={false}
        ...
        <div>
            <div className="chart-group__header">
                <div className="flexbox flex-justify  w-100">
                    <form
                        onSubmit={(e) => handleFilterQueryChanges(e, 'search')}
                        style={{ width: 'none' }}
                        className="search dc__position-rel"
                    >
                        ...
                        <input
                            ...
                            onChange={(e) => handleAppStoreName(e.target.value)}
                        />
                        {searchApplied ? (
                            <button
                                className="search__clear-button"
                                type="button"
                                onClick={(e) => handleFilterQueryChanges(e, 'clear')}
                            >
                    ...
                    <div className="flex">
                        <ReactSelect
                            className="date-align-left fs-13 pr-16"
