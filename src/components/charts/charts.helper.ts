...
export function getManageValuesURL(chartId): string {
    return `${URLS.CHARTS}/discover/chart/${chartId}/manage-values`;
}

export function getChartValuesURL(chartId: number | string, chartValueId?: number | string): string {
    if (chartValueId) { //update
        return `${URLS.CHARTS}/discover/chart/${chartId}/chart-value/${chartValueId}`;
    }
    else return `${URLS.CHARTS}/discover/chart/${chartId}/chart-value`; //create
}
