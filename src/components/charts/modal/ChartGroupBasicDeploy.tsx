        ...
        <div
            ...
            {charts.map((chart, index) => {
                if (chart.isEnabled) return <div key={index} className="form__row deploy-selected-chart__list-item">
                    <img onError={handleImageError} src={chart.chartMetaData.icon || ""} className="chart-grid-item__icon" />
                    <div className="w-100">
                        <span className="form__label form__label--lower">{chart.chartMetaData.chartRepoName}/{chart.chartMetaData.chartName}</span>
                        <input className={`form__input ${chart?.name?.error ? 'form__input--error' : ''}`} type="text" tabIndex={index + 1} value={chart.name.value}
                            onChange={(event) => {
