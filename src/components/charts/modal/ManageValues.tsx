...
class ManageValues extends Component<ManageValuesProps, ManageValuesState>{
    ...
    getChartValuesList() {
        getChartValuesTemplateList(this.props.chartId).then((response) => {
            let list = response.result || [];
            list = list.map((item) => { return { ...item, isLoading: false } });
    ...
    onEditChartValue(chartValueId: number) {
        let link = `${URLS.CHARTS}/discover/chart/${this.props.chartId}/chart-value/${chartValueId}`;
        this.props.history.push(link);
    }
    ...
    deleteChartValue(chartValueId: number) {
        ...
        deleteChartValues(chartValueId).then((response) => {
    ...
    renderHeader() {
        return <div className="modal__header modal__header--manage-values">
    ...
    renderCreateNew() {
        return <div className="manage-values__create-new" onClick={(event) => { this.onCreateNewChartValue() }}>
            <Add className="icon-dim-24 mr-5" />Create Custom
        </div>
    }
            ...
            <div className={`modal__body modal__body--manage-values`}>
                ...
                <ul className="manage-values__list">
                    {this.state.values.map((value) => {
                        return <li key={value.id} className="manage-values__list-item">
                            <div className="manage-values__chart-values-name ellipsis-right">{value.name} ({value.chartVersion})</div>
                            {value.isLoading ? <span className="align-right"><Progressing /></span>
                                : <div className="manage-values__item-action align-right">
                                    <button type="button" className="transparent mr-16"
                                        onClick={(event) => { this.onEditChartValue(value.id) }}>
                                        <Edit className="icon-dim-24" />
                                    </button>
                                    <button type="button" className="transparent"
                                        onClick={(event) => { this.deleteChartValue(value.id) }}>
