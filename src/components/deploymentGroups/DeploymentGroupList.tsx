...
export default class DeploymentGroupList extends Component<BulkActionListProps, BulkActionListState>{
    ...
    deploymentGroupList() {
        deploymentGroupList().then((response) => {
            let view = ViewType.FORM;
            if (response.result.length === 0) view = ViewType.EMPTY
            this.setState({ view: view, list: response.result, deploymentGroupId: 0, showGroupDeleteModal: false, showCDModal: false });
        }).catch((error) => {
    ...
    getCDMaterialList(deploymentGroup) {
        getCDMaterialList(deploymentGroup.id).then((response) => {
            if (response.result) {
                this.setState({
    ...
    triggerDeploy() {
        ...
        triggerGroupDeploy(request).then((response) => {
    ...
    deleteDeploymentGroup() {
        deleteDeploymentGroup(this.state.deploymentGroupId).then((response) => {
            if (response.result) {
    ...
    renderListItems() {
        return this.state.list.map((deploymentGroup) => {
            return <div key={deploymentGroup.id} className="bulk-action-list__row">
    ...
    renderGroupName(deploymentGroup) {
        return <div className="bulk-action-list__cell bulk-action-list__cell--name">
            <Link to={`${URLS.DEPLOYMENT_GROUPS}/${deploymentGroup.id}/details`} className="deployment-group-title" >{deploymentGroup.name}</Link>
            <div className="deployment-group__desc">Connected to {deploymentGroup.appCount} apps on {`'${deploymentGroup.environmentName}'`}</div>
        </div>
    }

    renderMaterials(materials: any[]) {
        return <div className="bulk-action-list__cell bulk-action-list__cell--source">
            {materials?.map((mat, idx) => {
                return <p key={idx} className="deployment-group__repo-list">
    ...
    renderActions(deploymentGroup) {
        return <div className="bulk-action-list__cell bulk-action-list__cell--action">
            <div className="deployment-group__actions" onClick={event => event.stopPropagation()}>
                <Tippy className="default-tt" content="Select Image to deploy">
                    <button type="button" className="transparent deployment-group__deploy mr-16"
                        onClick={(event) => { event.stopPropagation(); this.getCDMaterialList(deploymentGroup); }}>
                ...
                <PopupMenu autoClose>
                    ...
                    <PopupMenu.Body>
                        <ul className="kebab-menu__list">
                            <li key="edit" className="kebab-menu__list-item" onClick={(event) => { this.redirectToEdit(deploymentGroup) }}>Edit</li>
                            <li key="delete" className="kebab-menu__list-item kebab-menu__list-item--delete"
                                onClick={(event) => { this.setState({ deploymentGroupId: deploymentGroup.id, showGroupDeleteModal: true }, () => { }) }}>
                                Delete<Delete className="icon-dim-20" />
                            </li>
                        </ul>
                    </PopupMenu.Body>
                </PopupMenu>
    ...
    renderCDMaterial() {
        if (this.state.showCDModal) {
            return <CDMaterial material={this.state.materials}
                ...
                closeCDModal={() => { this.setState({ showCDModal: false }) }}
            />
        }
    }
    ...
    renderDeleteDialog() {
        ...
        if (this.state.showGroupDeleteModal)
            return <DeleteDialog title={group.name}
                description="Are you sure you want to delete this deployment group."
                closeDelete={() => { this.setState({ showGroupDeleteModal: false }) }}
                delete={() => { this.deleteDeploymentGroup() }} />
    }
        ...
        return(
            <div className="deployment-group-list-page">
                <div className="page-header">
                    <div className="page-header__title flex left"> Deployment Groups</div>
                    {this.state.view === ViewType.FORM && <div className="page-header__cta-container flex right"> <Link to={`${URLS.DEPLOYMENT_GROUPS}/0/edit`} className="cta align-right flex cta--link">
                ...
                <div className="deployment-group-list-page__body">
                    ...
                    {![ViewType.EMPTY, ViewType.ERROR, ViewType.LOADING].includes(this.state.view) &&  <React.Fragment>
                        {this.renderList()}
