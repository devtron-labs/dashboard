...
export const BulkActionEdit = () => {
            ...
            switch (action.type) {
                ...
                case 'selectApplication':
                    ...
                    return {
                        ...state,
                        prevEnvId: envId,
                        selectedApplications: Array.isArray(state.selectedApplications[envId])
                            ? {
                                  ...state.selectedApplications,
                                  [envId]: [...state.selectedApplications[envId], ...action.value],
                              }
                ...
                case 'unselectApplication':
                    return {
                        ...
                        selectedApplications: {
                            ...state.selectedApplications,
                            [state.environmentId]: state.selectedApplications[state.environmentId].filter(
                                (id) => id !== action.value,
                            ),
                        },
                    }
                case 'selectAllApplications':
                    return {
                        ...
                        selectedApplications: {
                            ...state.selectedApplications,
                            [state.environmentId]: state.applications
                                .filter((app) => app.envId === state.environmentId)
                                .map((app) => app.id),
                        },
                    }
    ...
    const selectedAppIds =
        state.environmentId && Array.isArray(state.selectedApplications[state.environmentId])
            ? state.selectedApplications[state.environmentId]
            : []
    ...
    async function createDeploymentGroup(e) {
        ...
        if (
            !state.selectedApplications[state.environmentId] ||
            state.selectedApplications[state.environmentId].length === 0
        ) {
        ...
        try {
            ...
            const nextLocation = location.pathname.split('/')
            nextLocation.pop()
    ...
    return (
        <div className="bulk-action-page bulk-action-page--edit">
                ...
                <div className="form-container">
                    ...
                    <div className="form__label">
                        ...
                        <input
                            ...
                            onChange={(e) => dispatch({ type: 'editGroupName', value: e.target.value })}
                            placeholder="Deployment group"
                            className="form__input"
                        />
                        {state.groupNameError && (
                            <label className="form__error flex left">
                                <Info color="#f33e3e" style={{ height: '16px', width: '16px' }} />
                                {state.groupNameError}
                            </label>
                        )}
                    </div>
                                    ...
                                    <>
                                        {state.pipelines.map((p) => (
                                            <PipelineSelect
                                                key={p.ciPipelineId}
                                                {...p}
                                                isActive={p.ciPipelineId === state.ciPipelineId}
                                                select={
                                                    Number(id) === 0
                                                        ? (e) =>
                                                              dispatch({
                                                                  type: 'selectPipeline',
                                                                  value: p.ciPipelineId,
                                                              })
                                                        : () => {}
                                                }
                                            />
                                ...
                                <div className="environments-select">
                                    ...
                                    {environments.map((env) => (
                                        <EnvSelect
                                            ...
                                            appCount={
                                                Array.isArray(state.selectedApplications[env.envId])
                                                    ? state.selectedApplications[env.envId].length
                                                    : 0
                                            }
                                            select={
                                                Number(id) === 0
                                                    ? (e) => dispatch({ type: 'selectEnvironment', value: env.envId })
                                                    : () => {}
                                            }
                                        />
                                    ))}
                                            ...
                                            {Array.isArray(state.applications) &&
                                                state.applications.map((app: any) => (
                                                    <AppSelect
                                                        ...
                                                        {...app}
                                                        selected={
                                                            Array.isArray(selectedAppIds) &&
                                                            selectedAppIds.includes(app.id)
                                                        }
                                                        select={(e) => handleSelectApp(app.id, app.name)}
                                                        selectedEnvId={state.environmentId}
                                                        searchString={state.appSearchString}
                                                    />
                                                ))}
                                        </div>
            ...
            <section className="cta-section flex">
                <button className="cta" onClick={createDeploymentGroup} type="button">
                    {state.loading ? <Progressing /> : 'Save Changes'}
                </button>
            </section>
            {state.confirmationDialog && (
                <ConfirmationDialog>
                    ...
                    <ConfirmationDialog.Body
                        title="Warning"
                        subtitle="You can create deployment groups for only one environment. Do you want to reset you previous selections?"
                    />
                    <ConfirmationDialog.ButtonGroup>
                        <div className="flex right">
                            <button
                                ...
                                onClick={(e) => dispatch({ type: 'closeModalDialog' })}
                            >
                                Cancel
                            </button>
                            <button type="button" className="cta" onClick={(e) => dispatch({ type: 'resetSelection' })}>
                                Reset Selection
                            </button>
                        </div>
                    </ConfirmationDialog.ButtonGroup>
            ...
            <div className="flex left top">
                ...
                <div className="pipeline--details flex left column w-100">
                    <div className="pipeline--info">
                        <b>{name}</b> (connected to {connections} Deployment pipelines)
                    </div>
        ...
        <div className="white-card--titled" onClick={onClick}>
            <div className={`title-container ${completed ? 'active' : ''}`}>
                ...
                <div className="next flex">
                    {completed && typeof next === 'function' && (
                        <button className="cta flex" onClick={next} type="button">
                            Next
                        </button>
                    )}
                </div>
