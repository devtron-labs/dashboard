...
export default function BulkActionDetails() {
    ...
    const {
        result: { appContainers, appCount, deploymentGroup },
    } = result

    async function onClickDeploy() {
        const materials = await getCDMaterialList(deploymentGroupId)
    ...
    async function triggerDeploy() {
        setIsLoading(true)
        const material = materials.find((mat) => mat.isSelected)
        const request = {
    ...
    function toggleSourceInfo(index: number) {
        const updatedMaterials = materials.map((mat, i) => {
            return {
                ...mat,
    ...
    function getConfirmationDialogTitles() {
        switch (deleteConfirmation) {
            case 'delete':
                return {
                    title: `Delete ${deploymentGroup.name}`,
                    subtitle: 'Are you sure you want to delete this deployment group ?',
                }
            case 'hibernate':
                return {
                    title: `Hibernate all applications`,
                    subtitle: (
                        <p>
                            Pods for all applications in this group will be <b>scaled down to 0 on this</b> environment.
                        </p>
                    ),
                }
            case 'resume':
    ...
    return (
        <>
            <section className="page deployment-detail-page">
                {deploymentGroup && (
                    <article className="dc__page-header">
                        <div className="dc__page-header__title flex left">{deploymentGroup.name}</div>
                        <div className="dc__page-header__cta-container flex left">
                            <button
                                className="cta-with-img cancel mr-8"
                                onClick={(event) => {
                                    onClickDeploy()
                                }}
                            >
                ...
                <div className="deployment-details-body">
                    <article className="deployment-details-metadata white-card flex left top">
                        <div className="sources flex left column">
                            ...
                            {(deploymentGroup?.ciMaterialDTOs || []).map(({ name, type, value }) => (
                                <MaterialList key={name} {...{ name, type, value }} />
                            ))}
                        </div>
                ...
                <ConfirmationDialog>
                    ...
                    <ConfirmationDialog.ButtonGroup>
                        <div className="flex right">
                            <button type="button" className="cta cancel" onClick={(e) => setDeleteConfirmation('')}>
                                Cancel
                            </button>
                            <button
                                type="button"
                                className={`cta ${deleteConfirmation}`}
                                disabled={pausing}
                                onClick={handleDelete}
                            >
                                {pausing ? <Progressing /> : getCTA()}
                            </button>
                        </div>
                    </ConfirmationDialog.ButtonGroup>
                </ConfirmationDialog>
    ...
    return (
        <div style={{ overflow: 'hidden', padding: '0' }} className="white-card">
            <table className="group-details">
                ...
                <tbody>
                    {apps.map(({ appId, appName, environments }) =>
                        environments.map(
                            ({
                                environmentId,
                                environmentName,
                                ...
                                return (
                                    <tr key={appId} className="pointer dc__striped-row">
                                        <Td to={newRoute}>{appName}</Td>
                                        <Td to={newRoute}>
                                            <MultiStatus
                                                {...{
                                                    pre: preStageStatus,
                                                    post: postStageStatus,
                                                    deploy: cdStageStatus,
                                                }}
                                            />
                                        </Td>
                                        ...
                                        <Td to={newRoute}>
                                            {lastDeployedTime
                                                ? moment(lastDeployedTime, 'YYYY-MM-DD HH:mm:ss')
                                                      .add(5, 'hours')
                                                      .add(30, 'minutes')
            ...
            <div className="status-progress flex">
                {[pre, deploy, post].map((stage) => (
                    <ConditionalWrap
                        condition={!!stage}
                        wrap={(children) => (
                            <Tippy className="default-tt" content={stage}>
                                {children}
                            </Tippy>
                        )}
                    >
