        ...
        <div className="flex left flex-wrap dc__gap-8">
            {labelTags.length > 0 ? (
                labelTags.map((tag,index) => (
                    <div key={tag.key} className="flex">
                        <div
                        ...
                        >
                            ...
                            <Tippy
                                ...
                                interactive={true}
                            >
                                <div className="dc__mxw-400 dc__ellipsis-right" data-testid={`tag-key-overview-${index}`}>{tag.key}</div>
                            </Tippy>
                        </div>
                        {tag.value && (
                            <Tippy
                                ...
                                interactive={true}
                            >
                                <div className="bcn-0 cn-9 fw-4 fs-12 en-2 bw-1 pr-6 pl-6 pb-2 pt-2 dc__right-radius-4 dc__no-left-border dc__mxw-400 dc__ellipsis-right"  data-testid={`tag-value-overview-${index}`}>
                                    {tag.value}
                                </div>
                            </Tippy>
                        )}
                ...
                <span className="fs-13 fw-4 cn-7" data-testid="overview-tags-value">No tags added.</span>
            )}
        </div>
    )
}
