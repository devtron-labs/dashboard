...
export class MaterialSource extends Component<MaterialSourceProps> {
    ...
    renderMaterialUpdateInfo(material) {
        if (material.isMaterialLoading) {
            return <div className="material-last-update">
            ...
            return <div className="material-last-update">
                ...
                <div className="material__error ellipsis-right">{material.isRepoError ? material.repoErrorMsg : material.isBranchError ? material.branchErrorMsg : ""}</div>
            </div>
        }
        else {
    ...
    renderRefreshButton(material) {
        return <button type="button" className="material-refresh"
            disabled={material.isMaterialLoading}
            onClick={(event) => {
                event.stopPropagation();
                this.props.refreshMaterial.refresh(this.props.refreshMaterial.pipelineId, this.props.refreshMaterial.title, material.gitMaterialId)
            }}>
    ...
    render() {
        return <>
            {this.props.material.map((material, index) => {
                ...
                return <div key={index} className={`material-list__item ${selectedClass}`} onClick={(e) => {
                    e.stopPropagation();
                    ...
                    <div className="branch-name ">
                        <CiPipelineSourceConfig sourceType={material.type} sourceValue={material.value} showTooltip={true} baseText={""} showWebhookIcons={true} />
                    </div>
