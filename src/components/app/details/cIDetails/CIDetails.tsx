...
export default function CIDetails() {
    ...
    async function pollHistory() {
        ...
        const [error, result] = await asyncWrap(getTriggerHistory(+pipelineId, { offset: 0, size: pagination.offset + pagination.size }))
        if (error) {
            showError(error)
            return
        }
                ...
                <div className="ci-details__history">
                    {!fullScreenView && <>
                        ...
                        <div className="flex column top left" style={{ overflowY: 'auto' }}>
                            {Array.from(triggerHistory).sort(([a,], [b,]) => (b - a)).map(([triggerId, trigger]) => <BuildCard key={trigger.id} triggerDetails={trigger} />)}
                            {hasMore && <DetectBottom callback={reloadNextAfterBottom} />}
                        </div>
                    </>}
                </div>
                <div ref={ref} className="ci-details__body">
                    {!pipelineId && <><div /><SelectPipelineView /></>}
                    {!!pipeline && <><div />
                        {pipeline.parentCiPipeline ? <LinkedCIPipelineView pipeline={pipeline} />
                    ...
                    {!!pipelineId && pipelineId === dependencyState[0] && triggerHistory?.size > 0 &&
                        <Route path={`${path.replace(":pipelineId(\\d+)?", ":pipelineId(\\d+)")}/:buildId(\\d+)?`}>
                            <BuildDetails fullScreenView={fullScreenView} triggerHistory={triggerHistory} pipeline={pipeline} setFullScreenView={setFullScreenView} synchroniseState={synchroniseState} />
                        </Route>
                    }
...
export const BuildCard: React.FC<{ triggerDetails: History }> = React.memo(({ triggerDetails }) => {
    ...
    return (
        <ConditionalWrap
            ...
            wrap={children =>
                <TippyHeadless placement="right" interactive
                    render={() =>
                        <BuildCardPopup triggerDetails={triggerDetails} />}>
                    {children}
                </TippyHeadless>
            }
        >
...
export const CIListItem: React.FC<{ type: 'report' | 'artifact', children: any }> = ({ type, children }) => {
    return <div className={`mb-16 ci-artifact ci-artifact--${type}`}>
...
const BuildDetails: React.FC<BuildDetails> = ({ triggerHistory, pipeline, fullScreenView, setFullScreenView, synchroniseState }) => {
    ...
    return <Details pipeline={pipeline} fullScreenView={fullScreenView} setFullScreenView={setFullScreenView} synchroniseState={synchroniseState} triggerHistory={triggerHistory} />
}
    ...
    return (
        <>
            <div className="trigger-details-container">
                {!fullScreenView && <>
                    <TriggerDetails triggerDetails={triggerDetails} type="CI" abort={() => cancelCiTrigger({ pipelineId, workflowId: buildId })} />
                    <ul className="ml-20 tab-list tab-list--borderd mr-20">
                        <li className='tab-list__tab'>
                            <NavLink replace className="tab-list__tab-link" activeClassName="active" to={`logs`}>Logs</NavLink>
                        </li>
                        <li className='tab-list__tab'>
                            <NavLink replace className="tab-list__tab-link" activeClassName="active" to={`source-code`}>Source code</NavLink>
                        </li>
                        <li className='tab-list__tab'>
                            <NavLink replace className="tab-list__tab-link" activeClassName="active" to={`artifacts`}>Artifacts</NavLink>
                        </li>
                        <li className="tab-list__tab">
                            <NavLink replace className="tab-list__tab-link"
            ...
            <HistoryLogs key={triggerDetails.id} pipeline={pipeline} triggerDetails={triggerDetails} setFullScreenView={setFullScreenView} />
        </>
    )
}

export const TriggerDetails: React.FC<{ triggerDetails: History, abort?: () => Promise<any>, type: 'CI' | 'CD' }> = ({ triggerDetails, abort, type }) => {
    const { url, path } = useRouteMatch()
    const { pathname } = useLocation()
    return (
            ...
            <div className="trigger-details__summary" style={{ display: 'grid', gridTemplateRows: '1fr 1fr' }}>
                <div className="trigger-details__start flex column left">
                    ...
                    <div className="flex left">
                        ...
                        {!pathname.includes('source-code') && <Link to={`${url}/source-code`} className="anchor ml-8">Commit details</Link>}
                    </div>
                </div>
                {
                    {
...
const Finished: React.FC<{ triggerDetails: History, colorClass: string, type: 'CI' | 'CD' }> = ({ triggerDetails, colorClass, type }) => {
    return (
        <div className="flex column left">
        ...
        <>
            ...
            <div className="flex left column">
                <div className="flex left fs-14">
                    ...
                    {triggerDetails.podStatus && <div className="fw-6" style={{ color: colorMap[triggerDetails.podStatus.toLowerCase()] }}>{triggerDetails.podStatus}</div>}
                </div>
...
const ProgressingStatus: React.FC<{ triggerDetails: History, abort?: () => Promise<any>, type: 'CI' | 'CD' }> = ({ triggerDetails, abort, type }) => {
    ...
    return (
        <>
            <div className="trigger-details__current flex left">
                <div style={{ color: '#ff7e5b' }} className={`${triggerDetails.status} fs-14 fw-6 flex left`}>In progress</div>
                {abort && <button className="cta cancel ml-16" style={{ minWidth: '72px' }} onClick={e => setAbortConfiguration(true)}>Abort</button>}
                <WorkerStatus triggerDetails={triggerDetails} />
            </div>
            {abortConfirmation &&
                <ConfirmationDialog>
...
const HistoryLogs: React.FC<{ pipeline: CIPipeline, triggerDetails: History, setFullScreenView: (...args) => void }> = ({ pipeline, triggerDetails, setFullScreenView }) => {
    ...
    return <>
        <div className="trigger-outputs-container">
            {pipeline.pipelineType === 'LINKED' ? <LinkedCIPipelineView pipeline={pipeline} />
                : <Switch>
                    ...
                    <Route path={`${path}/artifacts`} render={props => <Artifacts triggerDetails={triggerDetails} getArtifactPromise={() => getArtifact(pipelineId, buildId)} />} />
                    <Route path={`${path}/security`} render={props => <SecurityTab triggerHistory={triggerDetails} />} />
                    <Redirect to={triggerDetails.status.toLowerCase() === 'succeeded' ? `${path}/artifacts` : `${path}/logs`} />
                </Switch>}
        </div>
        {(scrollToTop || scrollToBottom) && <Scroller style={{ position: 'fixed', bottom: '25px', right: '32px' }} {...{ scrollToTop, scrollToBottom }} />}
    </>
...
const SelectPipeline: React.FC<Pipelines> = ({ pipelines }) => {
    ...
    return <div className="select-pipeline-wrapper w-100 pl-16 pr-16" style={{ overflow: 'hidden' }}>
        ...
        <Select onChange={handlePipelineChange} value={+pipelineId}>
            ...
            {pipelines.map((item, idx) => {
                return <Select.Option key={idx} value={item.id}>
...
const LinkedCIPipelineView: React.FC<{ pipeline: CIPipeline }> = ({ pipeline }) => {
    ...
    return (
        <EmptyState >
            ...
            <EmptyState.Title><h4>This is a Linked CI Pipeline</h4></EmptyState.Title>
...
function SelectPipelineView() {
    ...
    return (
        <EmptyState >
            <EmptyState.Image><img src={AppNotDeployed} alt="" /></EmptyState.Image>
            <EmptyState.Title><h4>No pipeline selected</h4></EmptyState.Title>
...
function NoTriggersView() {
    return (
        <EmptyState >
            <EmptyState.Image><img src={AppNotDeployed} alt="" /></EmptyState.Image>
            <EmptyState.Title><h4>Build pipeline not triggered</h4></EmptyState.Title>
            <EmptyState.Subtitle>Pipeline trigger history, details and logs will be available here.</EmptyState.Subtitle>
        </EmptyState>
    )
}
...
function CIProgressView() {
    return (
        <EmptyState >
            ...
            <EmptyState.Subtitle>Generated artifact(s) will be available here after the pipeline is executed.</EmptyState.Subtitle>
        </EmptyState>
    )
}
...
function NoArtifactsView() {
    return (
        <EmptyState >
            <EmptyState.Image><img src={AppNotDeployed} alt="" /></EmptyState.Image>
            <EmptyState.Title><h4>No artifacts generated</h4></EmptyState.Title>
            <EmptyState.Subtitle>Errr..!! We couldn’t build your code.</EmptyState.Subtitle>
        </EmptyState>
    )
}

export const LogsRenderer: React.FC<{ triggerDetails: History, setFullScreenView: (...args) => void }> = ({ triggerDetails, setFullScreenView }) => {
    ...
    return (
        <>
            <div className="logs__body">
                ...
                {eventSource && eventSource.readyState <= 1 && <div className="flex left event-source-status"><Progressing /></div>}
            </div>
                ...
                <div className="flex column left">
                    <div className="cn-9 fs-14 flex left visible-hover visible-hover--parent">
                        ...
                        <Tippy content={'Copy to clipboard'}>
                            <CopyIcon className="pointer visible-hover--child ml-6 icon-dim-16" onClick={e => copyToClipboard(triggerDetails.artifact.split(":")[1], () => toast.info('copied to clipboard'))} />
                        </Tippy>
                    </div>
                    <div className="cn-7 fs-12 flex left visible-hover visible-hover--parent">
                        {triggerDetails.artifact}
                        <Tippy content={'Copy to clipboard'}>
                            <CopyIcon className="pointer visible-hover--child ml-6 icon-dim-16" onClick={e => copyToClipboard(triggerDetails.artifact, () => toast.info('copied to clipboard'))} />
                        </Tippy>
                    </div>
                </div>
            </CIListItem>
            {getArtifactPromise && <CIListItem type="report">
                <div className="flex column left">
                    <div className="cn-9 fs-14">Reports.zip</div>
                    <button type="button" onClick={handleArtifact} className="anchor p-0 cb-5 fs-12 flex left pointer">
                        Download
...
const MaterialHistory: React.FC<{ gitTrigger: GitTriggers, ciMaterial: CiMaterial }> = ({ gitTrigger, ciMaterial }) => {
    return <div key={gitTrigger?.Commit} className="bcn-0 pt-12 br-4 en-2 bw-1 pb-12 mb-12" style={{ width: 'min( 100%, 800px )' }}>
        <GitCommitInfoGeneric
            materialUrl={gitTrigger.GitMaterialUrl ? gitTrigger.GitMaterialUrl :ciMaterial.url}
            showMaterialInfo={true}
            commitInfo={gitTrigger}
            materialSourceType={gitTrigger.GitSourceType ? gitTrigger.GitSourceType : ciMaterial.type}
            selectedCommitInfo={""}
            materialSourceValue={gitTrigger.GitSourceValue ? gitTrigger.GitSourceValue :ciMaterial.value} />
    </div>
}
...
const SecurityTab: React.FC<{ triggerHistory: History }> = (props) => {
    ...
    if (['starting', 'running'].includes(props.triggerHistory.status.toLowerCase())) return <CIRunningView isSecurityTab={true} />
    ...
    if (props.triggerHistory.artifactId && securityData.scanned && !securityData.vulnerabilities.length) return <NoVulnerabilityView />
    ...
    return <>
        ...
        <div className="white-card white-card--ci-scan">
            <div className="security-scan__header" onClick={toggleCollapse}>
                <Down style={{ ['--rotateBy' as any]: isCollapsed ? '0deg' : '180deg' }} className="icon-dim-24 rotate fcn-9 mr-12" />
                ...
                {severityCount.critical !== 0 ? <span className="fill-critical">{severityCount.critical} Critical</span> : null}
                {severityCount.critical === 0 && severityCount.moderate !== 0 ? <span className="fill-moderate">{severityCount.moderate} Moderate</span> : null}
                {severityCount.critical === 0 && severityCount.moderate === 0 && severityCount.low !== 0 ? <span className="fill-low">{severityCount.low} Low</span> : null}
            ...
            {isCollapsed ? '' : <>
                <table className="security-scan-table">
                    ...
                    {securityData.vulnerabilities.map((item) => {
                        return <tr className="security-scan-table__row">
                            <td className="security-scan-table__data security-scan-table__pl security-scan-table__cve cve-cell">
                                <a href={`https://cve.mitre.org/cgi-bin/cvename.cgi?name=${item.name}`} target="_blank">
                                    {item.name}
                                </a>
                            </td>
                            ...
                            <td className="security-scan-table__data security-scan-table__pl security-scan-table--w-18">{item.package}</td>
                            <td className="security-scan-table__data security-scan-table__pl security-scan-table--w-18">{item.version}</td>
                            <td className="security-scan-table__data security-scan-table__pl security-scan-table--w-18">{item.fixedVersion}</td>
                        </tr>
                    })}
                </table>
            </>}
