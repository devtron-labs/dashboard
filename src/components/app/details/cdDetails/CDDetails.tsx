...
export default function CDDetails(){
    const {appId, envId, triggerId, pipelineId} = useParams<any>()
    const [pagination, setPagination] = useState<{offset: number, size: number}>({offset: 0, size : 20})
    const [hasMore, setHasMore] = useState<boolean>(false);
    const [triggerHistory, setTriggerHistory] = useState<Map<number, History>>(new Map());

    const [fullScreenView, setFullScreenView] = useState<boolean>(false);
    const [loading, result, error] = useAsync(() => Promise.all([getAppOtherEnvironment(appId), getCDPipelines(appId)]), [appId])
    const [loadingDeploymentHistory, deploymentHistoryResult, deploymentHistoryError, reloadDeploymentHistory, ,dependencyState] = useAsync(() => getTriggerHistory(+appId, +envId, pipelineId, pagination), [pagination, appId, envId], !!envId && !!pipelineId)
    ...
    async function pollHistory(){
        ...
        const [error, result] = await asyncWrap(getTriggerHistory(+appId, +envId, +pipelineId, {offset: 0, size: pagination.offset +  pagination.size}))
        if(error){
            showError(error)
            return
        }
    ...
    useEffect(()=>{
        ...
        if(deploymentHistoryResult?.result?.length){
            const newUrl = generatePath(path, {appId, envId, pipelineId, triggerId: deploymentHistoryResult.result[0].id})
            push(newUrl)
        }
...
const DeploymentCard:React.FC<{triggerDetails: History}> = ({triggerDetails})=>{
    ...
    const {triggerId, ...rest} = useParams<any>()
    return (
        <ConditionalWrap
                ...
                <div className="flex column left ellipsis-right">
                    ...
                    <div className="flex left cn-7 fs-12">
                        ...
                        <span className="bullet bullet--d2 ml-4 mr-4"></span>
                            {triggerDetails.artifact && <div className="app-commit__hash app-commit__hash--no-bg"><img src={docker} className="commit-hash__icon grayscale" />{triggerDetails.artifact.split(":")[1].slice(-12)}</div>}
                        <span className="bullet bullet--d2 ml-4 mr-4"></span>
...
function SelectEnvironmentView(){
    return (
        <EmptyState >
            ...
            <EmptyState.Title><h4>No environment selected</h4></EmptyState.Title>
...
function NoCDTriggersView({environmentName}) {
    return (
        <EmptyState >
            ...
            <EmptyState.Subtitle>No deployment history available for the {environmentName} environment.</EmptyState.Subtitle>
        </EmptyState>
    )
}
...
function Logs({ triggerDetails }) {
    ...
    const { pipelineId, envId, appId } = useParams<any>()
        ...
        <>
            {triggerDetails.id === 0 ?
                <EmptyState>
                    ...
                    <EmptyState.Title><h4>No logs</h4></EmptyState.Title>
                    <EmptyState.Subtitle>Logs are only generated for pre-deployment and post-deployment stages</EmptyState.Subtitle>
                </EmptyState>
                :
                <div className="logs__body">
                    <div>
                        {logs.map(({ index, text }) => <p className="log mono fs-14" key={index} dangerouslySetInnerHTML={createMarkup(text)} />)}
                    </div>
                    {eventSrcRef.current && eventSrcRef.current.readyState <= 1 && <div className="flex left event-source-status"><Progressing /></div>}
                </div>
            }
        </>
    )
...
const TriggerOutput: React.FC<{
    ...
    const { appId, triggerId, envId, pipelineId } = useParams<any>();
...
const HistoryLogs: React.FC<{triggerDetails: History, loading: boolean}> = ({ triggerDetails, loading }) => {
    ...
    const {appId, pipelineId, triggerId, envId} = useParams<any>()
