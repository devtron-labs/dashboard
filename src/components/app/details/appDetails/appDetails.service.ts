...
export function isDatasourceConfigured(envName: string) {
    ...
    // const URL = `http://demo.devtron.info:32080/grafana/api/datasources/id/Prometheus-${envName}`;
...
export function isDatasourceHealthy(datasourceId: number | string) {
    ...
    // const URL = `http://demo.devtron.info:32080/grafana/api/datasources/proxy/${datasourceId}/api/v1/query?query=1&time=${timestamp.getTime()}`;
    return fetchAPI(URL, 'GET');
}