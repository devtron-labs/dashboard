...
export default function HelmAppList({serverMode, payloadParsedFromUrl, sortApplicationList, clearAllFilters}) {
    ...
    }, [JSON.stringify(externalHelmAppsList)])


    async function init(){
        /*// reset data starts
        let _clusterIdsCsv = _getClusterIdsFromRequestUrl();
        setDataStateType(AppListViewType.LOADING);
        setDevtronInstalledHelmAppsList([]);
        setFilteredHelmAppsList([]);
        setClusterIdsCsv(_clusterIdsCsv);
        ...
        getDevtronInstalledHelmApps(clusterIdsCsv).then((devtronInstalledHelmAppsListResponse : AppListResponse) => {
            ...
            if(clusterIdsCsv){
                let _sseConnection = new EventSource(`${Host}/application?clusterIds=${clusterIdsCsv}`, { withCredentials: true });
    ...
    function _onExternalAppDataFromSse(message : MessageEvent, _clusterIdsCsv : string, _sseConnection : EventSource){
        ...
        externalHelmAppsList.push(...externalAppData.result.helmApps)
        setExternalHelmAppsList(externalHelmAppsList);

        let _requestedSortedClusterIdsJson =  JSON.stringify(_clusterIdsCsv.split(',').sort((a, b) => a.localeCompare(b)));
        let _recievedSortedClusterIdsJson = JSON.stringify(_externalAppRecievedClusterIds.sort((a, b) => a.localeCompare(b)));
    ...
    function handleFilteration(){
        ...
        if (_projects?.length){
            _filteredDevtronInstalledHelmAppsList = _filteredDevtronInstalledHelmAppsList.filter(app => _projects.includes(app.projectId));
        }
        ...
        // handle search
        if(_search?.length){
            _filteredDevtronInstalledHelmAppsList = _filteredDevtronInstalledHelmAppsList.filter(app => app.appName.toLowerCase().includes(_search.toLowerCase()) || app.chartName.toLowerCase().includes(_search.toLowerCase()));
        }
    ...
    function _isAnyFilterationApplied(){
        return payloadParsedFromUrl.environments?.length || payloadParsedFromUrl.teams?.length || payloadParsedFromUrl.namespaces?.length || payloadParsedFromUrl.appNameSearch?.length;
    }
    ...
    function renderHeaders(){
        ...
        return <div className="app-list__header">
                    ...
                    <div className="app-list__cell app-list__cell--name">
                        ...
                        {
                            !dataFetchInProgress &&
                            <button className="app-list__cell-header" onClick={e => { e.preventDefault(); sortApplicationList('appNameSort') }}>App name
                                {sortBy == SortBy.APP_NAME ? <span className={`${sortOrder == OrderBy.ASC ? 'sort-up' : 'sort-down'}`}></span> : <span className="sort-col"></span>}
                            </button>
                        }
                    </div>
    ...
    function renderApplicationList(){
        return <>
            ...
            {filteredHelmAppsList.map((app) => {
                return <React.Fragment key={app.appId} >
                        <Link to={`/chart-store/deployments/${app.appId}/env/${app.environmentDetail.environmentId}`} className="app-list__row">
                            <div className="app-list__cell--icon">
                                <LazyImage className="chart-grid-item__icon" src={app.chartAvatar} onError={handleImageError} />
                            </div>
                            ...
                            <div className="app-list__cell app-list__cell--env">
                                <p className="truncate-text m-0">{app.environmentDetail.environmentName ? app.environmentDetail.environmentName : '-'}</p>
                            </div>
                            ...
                            <div className="app-list__cell app-list__cell--time">
                                {app.lastDeployedAt &&
                                <Tippy arrow={true} placement="top" content={handleUTCTime(app.lastDeployedAt, false)}>
                                    <p className="truncate-text m-0">{handleUTCTime(app.lastDeployedAt, true)}</p>
                                </Tippy>
                                }
                            </div>
                        </Link>
        ...
        return <div className="app-list">
            {
                ...
                <Empty view={AppListViewType.NO_RESULT}
                       title={"No apps found"}
    ...
    return (
        <>
            ...
            }
            {
                ...
                renderApplicationListContainer()
            }
        </>
      )
