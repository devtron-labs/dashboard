...
let _nodesSubject: BehaviorSubject<Node[]> = new BehaviorSubject([]);
let _nodesFilteredSubject: BehaviorSubject<Node[]> = new BehaviorSubject([]);
...
const IndexStore = {
    ...
    getPodsForRootNode: (rootNode: string): iNode[] => {
        let _nodes = _nodesSubject.getValue();
    ...
    getPodsRootParentNameAndStatus: (): [string, string][] => {
        let _nodes = _nodesSubject.getValue();
    ...
    getAllNewPods: () => {
        return _appDetailsSubject.getValue().resourceTree.podMetadata.filter((p) => p.isNew);
    },
...
export function getiNodesByKindWithChildNodes(_nodes: Node[], _kind: string): iNode[] {
...
export function getiNodesByRootNodeWithChildNodes(_nodes: Node[], rootNodes: iNode[]): iNode[] {
    ...
    const _nodesByParent = new Map<string, iNode[]>();
...
export function getPodsRootParentNameAndStatus(_nodes: Node[]): [string, string][] {
    ...
    let uniqueParents = podNodes
        ...
        .reduce((acc, val) => {
            acc.set(val[0], !acc.get(val[0]) ? val[1] : reduceKindStatus(acc.get(val[0]), val[1]));
            return acc;
    ...
    return Array.from(rootParents, ([name, value]) => [name, value] as [string, string]).sort(
        (a: [string, string], b: [string, string]) => (a[0] > b[0] ? 1 : -1),
    );
}
...
export const getPodsForRootNodeName = (_rootNode: string, _treeNodes: Node[]): iNode[] => {
