...
function EnvironmentSelectorComponent() {
    ...
    useEffect(() => {
        if (params.appId) {
            getAppOtherEnvironment(params.appId).then((response) => {
                        ...
                        <div style={{ minWidth: '200px' }}>
                            ...
                            {environments && environments.length > 0 &&
                                <Select
                                    ...
                                    options={Array.isArray(environments) ? environments.map(environment => ({ label: environment.environmentName, value: environment.environmentId })) : []}
                                    value={appDetails.environmentId ? { value: +appDetails.environmentId, label: appDetails.environmentName } : null}
                                    onChange={selected => {
                            ...
                            {(!environments || environments.length === 0) && appDetails &&
                                <div className='bw-1 eb-2 br-4 bcn-0 pl-12 pr-12 pt-4 pb-4' style={{ minWidth: '200px' }}>
