...
import { ErrorBoundary, Progressing, getLoginInfo, AppContext } from '..';
...
export default function Navigation() {
    ...
    return (
        <main>
            <VerticalNavigation history={history} match={match} location={location} />
            <div className="main">
                <Suspense fallback={<Progressing pageLoader />}>
                    <ErrorBoundary>
                        <Switch>
                            <Route path={[URLS.APP, URLS.EXTERNAL_APP]} render={() => <AppRouter />} />
    ...
    return (
        <ErrorBoundary>
            <AppContext.Provider value={{ environmentId, setEnvironmentId }}>
                <Switch>
                    <Route path={`${path}/:appId(\\d+)`} render={(props) => <AppDetailsPage />} />
                    <Route path={[`${path}/:appId(\\d+)/material-info`, path, ""]} render={(props) => <ListContainer {...props} closeModal={closeModal} />} />
                    {/* <Route path=""><ListContainer {...props} closeModal={closeModal} /></Route> */}
