...
export class Pagination extends Component<PaginationProps, PaginationState>{
    ...
    constructor(props) {
        ...
        let options = [{ value: 20, selected: true }, { value: 40, selected: false }, { value: 50, selected: false }];
        ...
        return <ul className="pagination" >
            <li className="page">
                <button className="page__button page__button--icon"
                    disabled={pageNoIndex === 0} onClick={() => { this.selectPrevPage() }}>
                    <span className="left-icon"><i className="fa fa-chevron-left"></i></span>
                </button>
            </li>
            {visiblePages.map((page, index) => {
                let classes = page.selected ? "page__button page__button--selected" : "page__button";
                return <li key={index} className="page">
                    <button className={classes} onClick={() => { this.selectPage(page.value) }}>
                        {page.value}
                    </button>
                </li>
            })}
            <li className="page">
                <button className="page__button page__button--icon"
                    disabled={pageNoIndex === this.state.pages.length - 1} onClick={() => { this.selectNextPage() }}>
                    <span className="left-icon"><i className="fa fa-chevron-right"></i></span>
                </button>
            </li>
        </ul>
    }
    ...
    renderDropdown() {
        ...
        return <div className="select">
            ...
            <div ref={node => this.node = node}>
                {this.state.show ? <div className="pagination__select-menu"> {this.state.options.map((option, index) => {
                    return <div key={index} className="select__item" onClick={() => { this.selectPageSize(option.value) }} >{option.value}</div>
                })}</div> : null}
                <button type="button" className="select__button" onClick={() => { this.setState({ show: !this.state.show }) }}>
                    <span>{pageSize ? pageSize.value : ""}</span>
                    <span className="select__icon"><i className={font}></i></span>
                </button>
            </div>
        </div>
    }
    ...
    renderPageInfo() {
        let end = this.props.offset + this.props.pageSize < this.props.size ? this.props.offset + this.props.pageSize : this.props.size;
        return <div className="page-number">{this.props.offset + 1} - {end} of {this.props.size}</div>
    }
