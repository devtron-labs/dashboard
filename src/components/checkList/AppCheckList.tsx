    ...
    render() {
        ...
        return <>
            ...
            <div className="fs-13 mb-12">
                {!hostUrl && <NavLink to={`${URLS.GLOBAL_CONFIG_HOST_URL}`} className="dc__no-decor mt-8 flex left" style={{ ['color']: hostUrl ? `#767d84` : `#0066cc` }} >
                    Add host URL</NavLink>}
                {!project && <NavLink to={`${URLS.GLOBAL_CONFIG_PROJECT}`} className="dc__no-decor  mt-8 flex left" style={{ ['color']: project ? `#767d84` : `#0066cc` }}>
                    Add project</NavLink>}
                {!environment && <NavLink to={`${URLS.GLOBAL_CONFIG_CLUSTER}`} className="dc__no-decor mt-8 flex left" style={{ ['color']: environment ? `#767d84` : `#0066cc` }}>
                    Add cluster & environment</NavLink>}
                {!git && <NavLink to={`${URLS.GLOBAL_CONFIG_GIT}`} className="dc__no-decor mt-8 flex left" style={{ ['color']: git ? `#767d84` : `#0066cc` }}>
                    Add git account</NavLink>}
                {!docker && <NavLink to={`${URLS.GLOBAL_CONFIG_DOCKER}`} className="dc__no-decor mt-8 pb-8 flex left" style={{ ['color']: docker ? `#767d84` : `#0066cc` }}>
