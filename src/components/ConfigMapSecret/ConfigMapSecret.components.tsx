...
export function ConfigMapSecretContainer({
    ...
    return (
        <section className="mb-12 br-8 bcn-0 bw-1 en-2 pl-20 pr-20 pt-19 pb-19">
            {collapsed ? (
                <article
                    className={`dc__configuration-list pointer ${data ? 'fw-5 cn-9 fs-14' : 'fw-5 cb-5 fs-14'}`}
                    onClick={updateCollapsed}
                >
                    {renderIcon()}
                    <div data-testid={`add-${componentType}-button`} className="configuration-list__info">
                        <div>{title}</div>
                    </div>
