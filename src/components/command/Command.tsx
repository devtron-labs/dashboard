...
export class Command extends Component<CommandProps, CommandState> {
    ...
    componentDidUpdate(prevProps, prevState) {
        ...
        if (
            this._inputPlaceholder.current &&
            (prevState.focussedArgument !== this.state.focussedArgument ||
    ...
    runCommand(): void {
        const focussedArg = this.state.suggestedArguments[this.state.focussedArgument]
        let allArgs = this.state.arguments
        const candidateArg = this.state.suggestedArguments.find((a) => a.value === this.state.argumentInput)
    ...
    navigate() {
        const last = this.state.arguments[this.state.arguments.length - 1]
        const args = this.state.arguments.reduce((acc, current) => {
    ...
    callGetArgumentSuggestions(args): void {
        ...
        this.setState({ isLoading: true }, async () => {
            try {
                ...
                const response = await getArgumentSuggestions(args, options)
                this._flexsearchIndex.clear()
                for (let i = 0; i < response.allSuggestionArguments.length; i++) {
                    this._flexsearchIndex.add(
                        response.allSuggestionArguments[i].value,
                        response.allSuggestionArguments[i].value,
                    )
                }

                const suggestedArguments = this.applyQueryOnSuggestions(
                    response.allSuggestionArguments,
                    this.state.argumentInput,
                )
                this.setState({
                    arguments: args,
                    suggestedArguments,
    ...
    isSuggestionInView(element): boolean {
        ...
        const container = this._menu
        const cTop = container.scrollTop
        const cBottom = cTop + container.clientHeight
        const eTop = element.offsetTop - 128
    ...
    handleKeyPress(event: KeyboardEvent) {
        if ((event.metaKey || event.ctrlKey) && event.key === 'k') {
            ...
            if (!this.state.argumentInput?.length) {
                const allArgs = this.state.arguments
                allArgs.pop()
            ...
            this.props.isCommandBarActive &&
            this.state.suggestedArguments.length &&
            event.key === 'ArrowRight'
        ) {
            const newArg = this.state.suggestedArguments[this.state.focussedArgument]
            if (!newArg) {
                return
            }
            ...
            const pos = (this.state.focussedArgument + 1) % this.state.suggestedArguments.length
            ...
            const pos =
                this.state.focussedArgument - 1 < 0
                    ? this.state.suggestedArguments.length - 1
                    : this.state.focussedArgument - 1
            if (!this.isSuggestionInView(this.state.suggestedArguments[pos]?.ref)) {
            ...
            const argInput = this.state.argumentInput.trim()
            const newArg = this.state.suggestedArguments.find((a) => a.value === argInput)
            let allArgs = []
            if (!newArg) {
                this.setState({
    ...
    handleArgumentInputChange(event) {
        if (event.target.value === '/') {
            ...
            const suggestedArguments = this.applyQueryOnSuggestions(
                this.state.allSuggestedArguments,
                event.target.value,
            )
            this.setState({
                argumentInput: event.target.value,
    ...
    applyQueryOnSuggestions(allSuggestedArguments, searchString: string): ArgumentType[] {
        if (!searchString) {
            return allSuggestedArguments
        }

        const argumentsMap = this.state.allSuggestedArguments.reduce((argumentsMap, arg) => {
            argumentsMap[arg.value] = arg.data
            return argumentsMap
        }, {})

        let suggestedArguments = []
        const results = this._flexsearchIndex.search(searchString)
    ...
    renderTabContent() {
        if (this.state.isLoading) {
            return (
                <div className="command__suggested-args-container mt-8">
                    <Progressing />
                </div>
            )
        }
                    ...
                    <div className="command" onClick={(event) => event.stopPropagation()}>
                        <div className="command-tab">
                            <div className="">
                                <label
                                    className={
                                        this.state.tab === 'this-app'
                                            ? 'command-tab__tab command-tab__tab-selected'
                                            : 'command-tab__tab'
                                    }
                                >
                                    <input
                                        type="radio"
                                        name="command-tab"
                                        checked={this.state.tab === 'this-app'}
                                        value="this-app"
                                        onKeyDown={this.disableArrowKeys}
                                ...
                                >
                                    <input
                                        type="radio"
                                        name="command-tab"
                                        checked={this.state.tab === 'jump-to'}
                                        value="jump-to"
                                        onKeyDown={this.disableArrowKeys}
                                        onChange={this.selectTab}
                                    />
                            ...
                            <div className="command-arg flex top w-100">
                                <div className="flex-1 flex left flex-wrap">
                                    {this.state.arguments.map((arg, index) => {
                                        return (
                                            <>
                                                <span key={`${index}-${arg.value}`} className="command-arg__arg m-4">
                                                    {arg.value}
                                                </span>
                                                {!arg.data?.isEOC ? (
                                    ...
                                    {!this.state.arguments[this.state.arguments.length - 1]?.data?.isEOC && (
                                        <div className="dc__position-rel m-4 flex-1" style={{ height: '22px' }}>
                                            <input
                                                ref={this._inputPlaceholder}
                                                type="text"
                                                placeholder={PlaceholderText}
                                                className="w-100 command__input"
                                            />
                                            <input
                                                ref={this._inputText}
                                                type="text"
                                                ...
                                                className="w-100 command__input"
                                                placeholder=""
                                                onKeyDown={this.noopOnArgumentInput}
                                                onChange={this.handleArgumentInputChange}
                                            />
                                        </div>
                                    )}
                                </div>
                        ...
                        {this.state.isSuggestionError ? (
                            <p className="command-empty-state__error pl-20 pr-20 pt-4 pb-4 mb-12">
                                Err! We couldn’t find anything by that name. Try one of the suggestions instead?
                            </p>
                        ) : null}
