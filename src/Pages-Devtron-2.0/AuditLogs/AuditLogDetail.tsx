...
const getFormattedPayload = (value: AuditLogDetailType['jsonFormatLog'] | AuditLogDetailType['payload']) => {
    if (!value) {
        return ''
    }
...
const getAuditLogPayload = ({ jsonFormatLog, payload }: AuditLogDetailType) => {
    const formattedJsonLog = getFormattedPayload(jsonFormatLog)

    return formattedJsonLog || getFormattedPayload(payload)
}

const AuditLogDetail = ({ auditLog, onClose }: AuditLogDetailProps) => {
    const payload = getAuditLogPayload(auditLog)
            ...
            <div className="flexbox-col h-100 bg__primary fs-13 cn-9">
                ...
                <div className="p-20 flexbox-col dc__gap-16 flex-grow-1 dc__overflow-hidden">
                    ...
                    <CodeEditor.Container flexExpand>
                        <CodeEditor
                            key={`${auditLog.auditLogId}-${payload}`}
                            readOnly
                            mode={MODES.JSON}
                            noParsing
                            value={payload}
                            height="fitToParent"
                        >
                            <CodeEditor.Header className="px-16 py-12 dc__border-bottom bg__secondary">
