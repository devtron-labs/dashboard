...
async function handleServerError(contentType, response) {
    ...
    serverError.errors = serverError.errors.length > 0 ? serverError.errors : [{ code, internalMessage: status, userMessage: status }]
    throw serverError
}

async function fetchAPI(url: string, type: string, data: object, signal: AbortSignal, preventAutoLogout = false): Promise<ResponseType> {
    let options = {
        method: type,
        headers: {
...
function fetchInTime(url: string, type: string, data: object, options?: APIOptions): Promise<ResponseType> {
    ...
    const timeoutPromise: Promise<ResponseType> = new Promise((resolve, reject) => {
        ...
        setTimeout(() => {
            controller.abort();
            reject({ code: 408, errors: [{ code: 408, internalMessage: 'Request cancelled', userMessage: 'Request Cancelled' }] });
        }, timeout)
    })
    return Promise.race([fetchAPI(url, type, data, options?.signal || signal, options?.preventAutoLogout || false), timeoutPromise]).catch(err => {

        if (err instanceof ServerErrors) {
            throw err
        }
        else {
            throw new ServerErrors({ code: 408, errors: [{ code: 408, internalMessage: 'That took longer than expected.', userMessage: 'That took longer than expected.' }] })
        }
    })
}

export const post = (url: string, data: object, options?: APIOptions): Promise<ResponseType> => {
    return fetchInTime(url, 'POST', data, options);
};
