...
import { AppConfigStatusResponseItem, EnvConfig, ResourceConfig } from '../../service.types'
import { AppStageUnlockedType, CustomNavItemsType, EnvResourceConfig, STAGE_NAME } from './appConfig.type'
...
export const getDTCMSecretList = (envConfigRes: EnvConfig, type: ResourceConfig['type']): EnvResourceConfig[] =>
    envConfigRes?.resourceConfig
        .filter((item) => item.type === type)
        .map(({ configState, name }) => ({
            configState,
            title: name,
        }))
