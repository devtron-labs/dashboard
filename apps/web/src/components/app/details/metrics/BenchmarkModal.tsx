    ...
    getGALabel() {
        switch (this.props.metric) {
            case 'DEPLOYMENT_FREQUENCY':
                return 'Average Deployment Frequency '
            case 'LEAD_TIME':
                return 'Mean Lead Time '
            case 'RECOVERY_TIME':
    ...
    getData() {
        if (this.props.metric === 'DEPLOYMENT_FREQUENCY') {
            return {
                ...
                elite: (
                    <>
                        <p>&gt; 1 /day</p> <p>Once per day.</p>
                    </>
                ),
                high: (
                    <>
                        <p>1 - 0.14 /day</p> <p>Once in 1 - 7 days.</p>
                    </>
                ),
                medium: (
                    <>
                        <p> 0.13 - 0.07 /day</p> <p>Once in 7 - 14 days.</p>
                    </>
                ),
                low: (
                    <>
                        <p>&lt; 0.06 /day</p> <p>Once in more than 14 days.</p>
                    </>
                ),
        ...
        return (
            <VisibleModal className="">
                <div className="modal__body" style={{ width: '800px' }}>
                    ...
                    <p className="graph-legend__primary-value flex left">
                        {this.props.valueLabel}
                        {this.props.metric === 'DEPLOYMENT_FREQUENCY' ? (
                            <>&nbsp; &bull; {this.renderCategoryDescription(this.props.value)}</>
                        ) : null}
                        {renderCategoryTag(this.props.category)}
                    </p>
                    ...
                    <table className="benchmark-table mb-24">
                        <tbody>
                            ...
                            <tr className="benchmark-table__row">
                                <td className="benchmark-table__cell">
                                    <b>{benchmark.metric}</b>
                                </td>
                    ...
                    <div className="pro-tip mb-20">
                        <p className="pro-tip__title m-0">
                            <Tip className="icon-dim-20 vertical dc__vertical-align-middle mr-8" /> Pro Tip
                        </p>
