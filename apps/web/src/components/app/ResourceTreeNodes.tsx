...
import { Pod as PodIcon, Trash } from '../common'
        ...
        >
            <div
                ...
                style={{
                    height: '32px',
                    border:
                        (!statusFilter && status.toLowerCase() === 'all') || ignoreCaseCompare(statusFilter, status)
                            ? '1px solid var(--B500)'
                            : '1px solid var(--N200)',
                    background:
                        (!statusFilter && status.toLowerCase() === 'all') || ignoreCaseCompare(statusFilter, status)
            ...
            >
                {status !== 'All' && (
                    <div
                        className={`dc__app-summary__icon icon-dim-16 mr-6 ${status.toLowerCase()} ${status.toLowerCase()}--node`}
                        style={{ zIndex: 'unset' }}
                    />
                )}
...
const ResourceTreeNodes: React.FC<ResourceTree> = ({
    nodes,
    describeNode,
    ...
    useEffect(() => {
        if (!searchParams.status || !params.kind) {
            return
        }
        outerloop: for (const kind of Object.keys(nodes.nodes)) {
            const kindNodes = nodes.nodes[kind]
            for (const [nodeName, nodeDetails] of kindNodes) {
                if ((nodeDetails?.status || nodeDetails?.health?.status) === searchParams.status) {
                    history.push(
                        `${generatePath(path, { ...params, kind: nodeDetails.kind })}?${queryParams.toString()}`,
                    )
                    break outerloop
                }
            }
    ...
    return (
        ...
        >
            <div className="flex left pl-24 pr-24 bcn-0" style={{ gridColumn: '1 / span 2' }}>
                <span className="fs-14 fw-6 cn-7 flex">
                    <CubeIcon className="icon-dim-20 mr-8 fcn-7" /> APPLICATION OBJECTS
                </span>
...
export const NodeGroup: React.FC<{ title: AggregationKeysType; data: Object; aggregatedNodes: AggregatedNodes }> = ({
    title,
    data,
    aggregatedNodes,
}) => {
        ...
        <>
            <>
                ...
                <div
                    data-testid={title}
                    onClick={(e) => setCollapsed(not)}
                    className="fs-14 pointer w-100 fw-6 flex left pl-8 pr-8"
                >
                    {title}
            ...
            {!collapsed &&
                Object.keys(data)
                    .filter(
                        (kind) =>
                            !filterStatus ||
                            (aggregatedNodes.nodeStatusCount[kind] &&
                                aggregatedNodes.nodeStatusCount[kind][filterStatus]),
                    )
                    .map((kind) => (
...
export const AllPods: React.FC<AllPods> = ({
    isAppDeployment,
    pods,
    describeNode,
    appName,
    ...
    const { newPods, oldPods, newPodStats, oldPodStats, allPodStats } = podsArray.reduce(
        (agg, curr, idx) => {
            ...
            {isAppDeployment ? (
                <>
                    <div className="flex left old-new-switch-container">
                        <div
                            className={`dc__no-decor old-new-link flex left column pl-16 pr-16 pointer ${
                                podTab === 'new' ? 'active' : ''
                            }`}
...
export const GenericInfo: React.FC<{
    appName: string
    environmentName: string
    nodes: AggregatedNodes
    level?: number
            ...
            {level === 1 && (
                <div className="flex left column w-100 generic-info-header">
                    <div style={{ height: '64px' }} className="pl-16 pr-16 flex column left">
                        <div className="fs-14 fw-6 cn-9">
                            {type} ({nodes.nodes[type].size})
                        </div>
                            ...
                            {nodes.nodeStatusCount[type] &&
                                Object.entries(nodes.nodeStatusCount[type]).map(([status, count], idx, arr) => (
                                    <React.Fragment key={status}>
                                        ...
                                        {idx !== arr.length - 1 && <span className="dc__bullet ml-6 mr-6" />}
                                    </React.Fragment>
                                ))}
                        </div>
                    </div>
...
export const NestedTable: React.FC<{
    appName: string
    environmentName: string
    level: number
    type: NodeType
    ...
    return (
        <table
            className={`resource-tree ${level === 1 ? 'ml-10' : ''}`}
            style={{ width: level === 1 ? 'calc( 100% - 10px )' : '100%' }}
        >
            <thead>
                <tr>
                    <th />
...
}> = ({ nodeDetails, describeNode, addExtraSpace }) => {
    const { path } = useRouteMatch()
    const history = useHistory()
        ...
        <td className="hover-trigger" data-testid={`${nodeDetails.name}-hover-trigger`}>
            <div className="flex left top" {...(addExtraSpace && { style: { marginRight: '48px' } })}>
                ...
                {nodeDetails.kind.toLowerCase() === 'pod' ? (
                    <span
                        data-testid={`${nodeDetails.name}-terminal`}
                        className="hover-only fw-6 anchor pointer ml-6"
                        ...
                        {nodeDetails.kind === Nodes.Pod && nodeDetails?.containers?.length ? (
                            <NestedTable
                                ...
                                Data={nodeDetails.containers.reduce((agg, containerName) => {
                                    agg.set(containerName, {
                                        name: containerName,
                                        kind: Nodes.Containers,
                                        pName: nodeDetails.name,
                                    })
                                    return agg
                                }, new Map())}
                                nodes={nodes}
...
const PodPopup: React.FC<{
    ...
    return (
        <div className="pod-info__popup-container">
            {kind === Nodes.Pod ? (
                <span className="flex pod-info__popup-row" onClickCapture={(e) => describeNode(NodeDetailTabs.EVENTS)}>
                    View Events
                </span>
            ) : (
                ''
            ...
            <span className="flex pod-info__popup-row pod-info__popup-row--red" onClick={asyncDeletePod}>
                <span>Delete</span>
...
export const NoPod = ({ selectMessage = 'Select a pod to view events', style = {} }) => {
    return (
...
export const EmptyPage = ({ title = 'Data not available' }) => {
    return (
        <div className="flex column" style={{ height: '100%', width: '100%' }}>
            ...
            <div
                className="subtitle"
                style={{ fontSize: '12px', color: 'var(--N700)', width: '200px', textAlign: 'center' }}
            >
                please modify configuration and redeploy.
            </div>
        </div>
    )
}
...
export function getAllContainers(nodeDetails) {
    const allContainers = nodeDetails.containers.concat(nodeDetails.initContainers)
